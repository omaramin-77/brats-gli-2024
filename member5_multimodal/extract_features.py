"""Member 5 — extract bottleneck features for symmetry with M1-M4.

Loads the trained M5 multimodal checkpoint, runs forward_features() on every
patient in train + val + test splits, and saves the resulting
(1, 256, 8, 8, 8) bottleneck tensors to disk.

Note: M5's Latent Fusion Head (Pipeline C) does NOT consume M5's own
features — it consumes M1-M4's per-modality features. This script exists
for completeness/symmetry with the other members.
"""
...
MODALITY = "multimodal"
FEATURES_DIR = RESULTS_DIR / "features" / "M5_Multimodal_4ch"
...
def main() -> None:
    ...
    # whole (4, 128, 128, 128) preprocessed volume — no patches.
    ds = BraTSDataset(
        data_root=data_root,
