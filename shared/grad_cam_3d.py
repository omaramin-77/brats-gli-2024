...
::
    ...
    cam = GradCAM3D(model, target_layer=model.encoder.bottleneck)
    heatmap = cam.generate(input_tensor)            # numpy array, same spatial shape as input
    cam.remove_hooks()                              # always release after use
"""
...
class GradCAM3D:
    ...
    def generate(
        ...
        """Compute a Grad-CAM heatmap aligned with ``input_tensor``.

        ``target_mask`` selects which voxels in the network output drive the
        backward pass. If None we use the predicted-positive voxels (sigmoid
        output > 0.5), which gives the canonical "what supports this
        ...
        if target_mask is None:
            with torch.no_grad():
                target_mask = (torch.sigmoid(output) > 0.5).float()
        else:
            target_mask = target_mask.float().to(output.device)

        # Reduce to a scalar so .backward() has something to differentiate.
        loss = (output * target_mask).mean()
