...
class ResidualUNet3D(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        """Return the bottleneck feature map at the shared common shape
        ``(B, 256, H/16, W/16, D/16)``. For 64³ input → ``(B, 256, 4, 4, 4)``;
        for 128³ input → ``(B, 256, 8, 8, 8)``. This is what M5's Latent Space
        Fusion Head consumes (design.md §9.2 binding contract)."""
        s1 = self.enc1(x)
...
if __name__ == "__main__":
    ...
    # --- forward_features → bottleneck at the shared common shape ---
